...
# magic numbers, either extracted from data or manually chosen
...
class OurCF:
    ...
    def __init__(self, shrink_term=1):
        ...
        self.dataset = None
        self.NN = NearestNeighbors(metric=custom_similarity)

    def fit(self, df):
        """
            takes in pandas dataframe with columns ['critic_id', 'movie_id', 'score']
            constructs the NearestNeighbors model with correlation distance
        """
        self.dataset = df
    ...
    def get_score(self, user_id, movie_id, neighbors):
        ...
        item_neighbors = self.NN.kneighbors([self.dataset.loc[movie_id]], n_neighbors=n_neighbors)
    ...
    def recommend(self, user_id, k):
        ...
        for movie in top_ranked_movies:
            movie_neighbors = self.NN.kneighbors([self.dataset.loc[movie]], n_neighbors=5)  # 5 is a magic number
...
if __name__ == '__main__':
    ...
    print('\nLoading data')
    df_reviews = pd.read_csv('reviews_clean.tsv', sep='\t')[['movie_id', 'critic_id', 'score']]
    features = json.load(open('films_features_raw.json'))
    one_hot_encoded_features = pd.read_csv('film_features_processed.tsv', sep='\t').set_index('movie_id', drop=True)
    ...
    df_reviews = df_reviews.append(me).reset_index(drop=True).pivot(index='movie_id', columns='critic_id', values='score').fillna(0)
    amount_critics = df_reviews.shape[1]
    final_df = pd.merge(df_reviews, one_hot_encoded_features, left_index=True, right_index=True).astype(int)
    print('Data loaded\n')
